...
- **[Week 6: Speak the Auditor's Language (OSCAL)](week-6/)** - A `trestle`-authored component definition and profile mapping **SC-28, AC-3, AU-3, and CM-6** to their Terraform implementations, with evidence links to the signed week-4 bundle. Traversal proven end to end: control → evidence `href` → `verify-evidence.sh` → `CHAIN INTACT`.
